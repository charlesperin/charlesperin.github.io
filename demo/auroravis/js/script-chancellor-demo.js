...
//var kinectActiveCutoff = 510; // min is 500
var kinectActiveCutoff = 500; // min is 500
...
function onKeydown(event) {
	switch (event.key) {
		...
		case 'ArrowRight':
			...
			event.preventDefault();
			break;
		case 'PageDown': // for remote
				seekVideo(20);
				event.preventDefault();
				break;
		case 'PageUp': // for remote
				seekVideo(-20);
				event.preventDefault();
				break;
		case 'Home':
		...
		case '.':
		  kinectOn = !kinectOn;
			if (!kinectOn) {
				currentPosition = -1;
				currentPositions = [];
				// Reset kinectActive so that touch hover works again
				kinectActive = Array(20).fill(false);
				$('#cursor')
					.removeClass('active')
					.removeClass('upper');
...
function onKinectUpdate(data) {
	...
	else // if we get body updates
	{
		...
		// 80% between middle of spine and should-level of spine
		...
		if((joints.HandLeft.Position.Y > threshold) || (joints.HandRight.Position.Y > threshold))
		{
			var halfWindowWidth = windowWidth / 2;
			var overElem;
			var left;

			if (joints.HandLeft.Position.Y > threshold) {
				// let's say min = 0 = joints.ElbowLeft.Position.X
				// let's say max = halfWindowWidth = joints.Head.Position.X
				// interpolate between both?
				var kinectStart = joints.ElbowLeft.Position.X;
				var kinectEnd = joints.Head.Position.X;
				var kinectRange = kinectEnd - kinectStart;
				var windowStart = 0;
				var windowEnd = halfWindowWidth;
				var windowRange = windowEnd - windowStart;

				left = windowStart + windowRange * ((joints.HandLeft.Position.X - kinectStart) / (kinectRange));
				// cut off at the sides of the screen!
				left = (left < windowStart) ? windowStart: left;
				left = (left > windowEnd) ? windowEnd: left;
				...
				$('#cursor')
					...
					.css('top', 25)
					.css('left', left);
				overElem = getOverElementHorizontal(left, buttons);
			} else if (joints.HandRight.Position.Y > threshold) {
				var kinectStart = joints.Head.Position.X;
				var kinectEnd = joints.ElbowRight.Position.X;
				var kinectRange = kinectEnd - kinectStart;
				var windowEnd = windowWidth;
				var windowStart = halfWindowWidth;
				var windowRange = windowEnd - windowStart;

				left = windowStart + windowRange * ((joints.HandRight.Position.X - kinectStart) / (kinectRange));

				// cut off at the sides of the screen!
				left = (left < windowStart) ? windowStart: left;
				left = (left > windowEnd) ? windowEnd: left;

				/*var kinectStart = joints.ElbowRight.Position.X;
				var kinectEnd = joints.Head.Position.X;
				var kinectRange = kinectEnd - kinectStart;
				var windowEnd = windowWidth;
				var windowStart = halfWindowWidth;
...
/*function handleKeyInput(keyboardEvent) {
	// STOP key on remote clicker
	if (keyboardEvent.keyCode == 190) {
		kinectOn = !kinectOn;
	}
}*/
